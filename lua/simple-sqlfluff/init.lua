...
local function render(violations)
  ...
  vim.diagnostic.set(vim.api.nvim_create_namespace("sqlfluff"), 0, prepared_violations)
end
...
local function fix()
  local fn = vim.fn.expand("%:p")
  local cmd = "sqlfluff fix " .. fn
  vim.fn.system(cmd)

  local fixed_content = vim.fn.readfile(fn)
  vim.api.nvim_buf_set_lines(0, 0, -1, false, fixed_content)
end
...
function M.setup(opts)
  ...
  vim.api.nvim_create_user_command("SQLFluffToggle", toggle_autocommand, { nargs = 0 })

  vim.api.nvim_create_user_command("SQLFluffEnable", function()
    sett.autocommand_toggle = true
  end, { nargs = 0 })

  vim.api.nvim_create_user_command("SQLFluffDisable", function()
    sett.autocommand_toggle = false
  end, { nargs = 0 })

  vim.api.nvim_create_user_command("SQLFluffFormat", format, { nargs = 0 })

  vim.api.nvim_create_user_command("SQLFluffFix", fix, { nargs = 0 })
end
